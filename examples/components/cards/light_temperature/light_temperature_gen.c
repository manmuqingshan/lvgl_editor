...
lv_obj_t * light_temperature_create(lv_obj_t * parent)
{
    ...
    if (examples_check_target(EXAMPLES_TARGET_ALL)) {
        ...
        lv_obj_bind_checked(switch_0, &light_temp);

        lv_obj_t * lv_arc_0 = lv_arc_create(card_0);
        lv_arc_bind_value(lv_arc_0, &light_temp_temp);
        ...
        lv_obj_bind_state_if_eq(lv_arc_0, &light_temp, LV_STATE_DISABLED, 0);
        ...
        lv_obj_bind_style(column_0, &disabled, 0, &light_temp, 0);
        lv_obj_t * lv_label_0 = lv_label_create(column_0);
        lv_label_bind_text(lv_label_0, &light_temp_temp, "%dK");
